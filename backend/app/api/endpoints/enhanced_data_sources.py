...
from app.models.enhanced_data_source import EnhancedDataSource, DataSourceType, SQLQueryType
from app.services.sql_query_builder import SQLQueryBuilder, JoinConfig, ColumnMapping, WhereCondition
...
def create_enhanced_data_source(
    *,
    ...
    """创建增强版数据源"""
    # 检查名称是否已存在
    if db.query(EnhancedDataSource).filter(EnhancedDataSource.name == data_source_in.name).first():
        raise HTTPException(status_code=400, detail="数据源名称已存在")
    
    # 验证SQL查询
    if data_source_in.source_type == DataSourceTypeEnum.sql and data_source_in.base_query:
        builder = SQLQueryBuilder()
        validation = builder.validate_query(data_source_in.base_query)
        if not validation["valid"]:
            raise HTTPException(status_code=400, detail=f"SQL查询验证失败: {validation['error']}")
...
def read_enhanced_data_source(
    *,
    ...
    """获取指定增强版数据源"""
    data_source = db.query(EnhancedDataSource).filter(EnhancedDataSource.id == data_source_id).first()
    if not data_source:
    ...
    *,
    ...
    try:
        ...
        if data_source.column_mapping:
            mappings = [ColumnMapping(**mapping) for mapping in data_source.column_mapping]
        else:
            mappings = []
        
        if data_source.where_conditions:
            conditions = [WhereCondition(**condition) for condition in data_source.where_conditions]
        else:
            conditions = []
