...
class RetrieveDataResponse(BaseModel):
    ...
    dataframe_str: str = Field(
        ..., description="便于LLM直接阅读的DataFrame字符串表示形式。"
    )


@router.post("/retrieve-data", response_model=RetrieveDataResponse)
async def retrieve_data(
    *, db: Session = Depends(deps.get_db), request: RetrieveDataRequest
):
    """
    ...
    # 这里我们简化为直接按名称查询
    data_source = (
        db.query(crud.data_source.model)
        .filter(crud.data_source.model.name == request.data_source_name)
        .first()
    )
    ...
    if not data_source:
        raise HTTPException(
            status_code=404,
            detail=f"Data source '{request.data_source_name}' not found.",
        )
    ...
    return {"data": df.to_dict(orient="records"), "dataframe_str": df.to_string()}
