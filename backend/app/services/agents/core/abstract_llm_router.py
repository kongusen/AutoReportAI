...
class AbstractModelType(Enum):
    ...
    WEB_SEARCH = "webSearch"        # 网页搜索 - 实时信息检索相关
...
class TaskContext:
    ...
    requires_realtime: bool = False        # 是否需要实时信息
...
class AbstractLLMRouter:
    ...
    def _build_routing_rules(self) -> Dict[str, List[AbstractModelType]]:
        ...
        return {
            ...
            "realtime_query": [AbstractModelType.WEB_SEARCH, AbstractModelType.DEFAULT],
    ...
    def route_to_abstract_model(self, task_context: TaskContext) -> AbstractModelType:
        ...
        try:
            # 1. 基于特殊需求的优先路由
            if task_context.requires_realtime:
                logger.info(f"实时任务路由: {AbstractModelType.WEB_SEARCH.value}")
                return AbstractModelType.WEB_SEARCH
    ...
    def get_model_config_for_abstract_type(self, abstract_type: AbstractModelType) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            default_mappings = {
                ...
                AbstractModelType.WEB_SEARCH.value: {
                    "provider": "openai",
                    "model_name": "gpt-4o-mini",
                    "max_tokens": 4096, 
                    "temperature": 0.4,
                    "description": "实时搜索和信息检索模型"
                }
            }
...
def analyze_task_requirements(content: str) -> Dict[str, bool]:
    ...
    return {
        ...
        "requires_realtime": any(keyword in content_lower for keyword in [
            "最新", "实时", "当前", "现在", "今天", "搜索", "查询"
        ]),
