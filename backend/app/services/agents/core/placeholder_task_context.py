"""
占位符任务上下文
用于传递placeholder上下文工程分析结果到agent系统
"""
...
class PlaceholderComplexity(Enum):
    """占位符任务复杂度"""
...
class PlaceholderTaskContext:
    """
    占位符任务上下文
    承载从placeholder domain构建的上下文工程数据（符合DAG架构）
    """
    # 基本信息
    task_id: str
    ...
    # 上下文工程分析结果（由background agent分析产生）
    ...
    # 任务特征 (从上下文分析中提取)
    complexity: PlaceholderComplexity = PlaceholderComplexity.MEDIUM
    ...
    def _evaluate_complexity(self):
        ...
        if complexity_score >= 7:
            self.complexity = PlaceholderComplexity.VERY_HIGH
        elif complexity_score >= 4:
            self.complexity = PlaceholderComplexity.HIGH
        elif complexity_score >= 2:
            self.complexity = PlaceholderComplexity.MEDIUM
        else:
            self.complexity = PlaceholderComplexity.SIMPLE
    ...
    def _generate_default_steps(self) -> List[ExecutionStep]:
        ...
        # 7. 验证步骤 (复杂任务需要)
        if self.complexity in [PlaceholderComplexity.HIGH, PlaceholderComplexity.VERY_HIGH]:
            steps.append(ExecutionStep(
                step_id=f"step_{step_counter}",
...
def create_task_context_from_placeholder_analysis(
    placeholder_text: str,
    statistical_type: str, 
    description: str,
    ...
    """
    从placeholder上下文工程分析结果创建任务上下文（符合DAG架构）
    
    Args:
        placeholder_text: 占位符文本
        statistical_type: 统计类型
        description: 需求描述
        context_analysis: background agent分析的上下文结果
        user_id: 用户ID
        context_engine: placeholder domain构建的上下文工程数据
    
    Returns:
        PlaceholderTaskContext实例
    """
    # 提取置信度
    ...
    logger.info(f"创建占位符任务上下文（DAG架构）: {task_context.get_task_summary()}, 复杂度: {task_context.complexity.value}")
