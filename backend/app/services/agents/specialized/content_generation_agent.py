...
class ContentRequest:
    ...
    content_type: str = "text"  # "text", "summary", "analysis", "description"
...
请生成{tone}的数据描述，不超过{max_length}字。
    ...
    async def execute(
        ...
        try:
            ...
            if content_request.data is None or (isinstance(content_request.data, (list, dict)) and len(content_request.data) == 0):
                raise AgentError(
                    "未提供内容生成所需的数据",
                    self.agent_id,
                    "NO_DATA_PROVIDED"
    ...
    async def _generate_text_content(self, request: ContentRequest) -> ContentResult:
        ...
        try:
            ...
            # Fallback to template-based generation if AI failed or unavailable
