...
class DataQueryAgent(BaseAgent):
    ...
    async def _get_data_source(self, data_source_id: int):
        ...
        if not HAS_DATABASE:
            raise RuntimeError("数据库连接不可用，无法获取数据源配置")
        ...
        try:
            ...
            return type('DataSource', (), {
                'id': data_source_id,
                'name': f'DataSource {data_source_id}',
                'source_type': 'database'
            })()
        except Exception as e:
            self.logger.error(f"获取数据源配置失败: {e}")
            raise RuntimeError(f"无法获取数据源配置: {e}")
    ...
    async def _execute_raw_sql(
        ...
        if not HAS_DATABASE:
            raise RuntimeError("数据库连接不可用，无法执行SQL查询")
        
        try:
            # Use the data source service to execute queries
            if hasattr(data_source_service, 'fetch_data'):
                result = await data_source_service.fetch_data(str(data_source.id), {"query": sql})
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        health["database_connection"] = "unavailable"
        return health