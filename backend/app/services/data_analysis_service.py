...
class DataAnalysisService:
    ...
    def analyze(self, data_source_id: int):
        ...
        try:
            # 获取数据源配置
            data_source = self.db_session.query(models.DataSource).filter(
        ...
        try:
            ...
            if chart_type == "bar":
                ...
                if not categorical_cols or not numeric_cols:
                    return {"error": "Insufficient data for bar chart (need categorical and numeric columns)"}
                ...
                chart_data = df.to_dict('records') if date_cols else [
                    {x_col: i, y_col: row[y_col]} for i, row in df.iterrows()
                ]
