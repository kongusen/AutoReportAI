...
from app.models.enhanced_data_source import DataSourceType, SQLQueryType
...
class EnhancedDataSourceService:
    ...
    async def create_data_source(
        ...
        try:
            with get_db_session() as db:
                ...
                # 创建数据源
                from app.schemas.enhanced_data_source import EnhancedDataSourceCreate

                source_data = EnhancedDataSourceCreate(
    ...
    async def test_connection(self, source_id: str) -> Dict[str, Any]:
        ...
        try:
            with get_db_session() as db:
                data_source = crud.enhanced_data_source.get(db, id=source_id)
                if not data_source:
