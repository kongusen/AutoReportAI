...
class MCPAnalyticsEngine:
    ...
    async def _calculate_period_comparison(
        self, context: AnalyticsContext, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """计算环比分析"""
        ...
        for col in value_cols:
            ...
            # 环比变化率
            period_change_rate = (
                period_change / previous_values.replace(0, np.nan)
            ) * 100
        ...
        for col in value_cols:
            ...
            for _, row in monthly_data.iterrows():
                ...
                if not last_year_data.empty:
                    ...
                    yoy_rate = (
                        (yoy_change / last_year_value) * 100
            ...
            results[col] = {
                ...
                "summary": {
                    "avg_yoy_change": (
                        np.mean([x["change"] for x in yoy_changes])
                        if yoy_changes
                        else 0
                    ),
                    "avg_yoy_rate": (
                        np.mean([x["rate"] for x in yoy_changes]) if yoy_changes else 0
                    ),
                    "positive_periods": len(
                        [x for x in yoy_changes if x["change"] > 0]
                    ),
    ...
    async def _calculate_summary_stats(
        ...
        if group_cols:
            ...
            for col in value_cols:
                stats = (
                    grouped[col]
                    .agg(["count", "sum", "mean", "std", "min", "max", "median"])
                    .reset_index()
                )
        ...
        for col in value_cols:
            ...
            results[col] = {
                ...
                "summary": {
                    ...
                    "total_growth": (
                        cumulative_growth.iloc[-1] if len(cumulative_growth) > 0 else 0
                    ),
    ...
    async def _calculate_proportion(
        self, context: AnalyticsContext, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """计算比例分析"""
        df = context.data.copy()
...
class MCPAnalyticsAPI:
    ...
    async def analyze_data(
        ...
        context = AnalyticsContext(
            ...
            period_type=(
                parameters.get("period_type", "daily") if parameters else "daily"
            ),
        )
        ...
        result = await self.engine.execute_analytics(
            operation=operation_type, context=context, parameters=parameters
        )
