...
from app.services.report_composition_service import ReportCompositionService
        ...
        try:
            ...
            for placeholder in placeholders:
                ...
                try:
                    ...
                    placeholder_key = f"{{{{{placeholder_name}}}}}" if placeholder_type == "scalar" else f"[{placeholder_type}:{placeholder_name}]"
                    placeholder_results[placeholder_key] = f"[Error processing {placeholder_name}: {str(e)}]"
            ...
            output_filename = f"report_{task.name}_{generation_result['generation_id']}.docx"
            ...
            generation_result["duration_seconds"] = (end_time - start_time).total_seconds()
    ...
    async def preview_report_data(
        ...
        try:
            ...
            for placeholder in placeholders:
                ...
                if not sample_data.empty:
                    for col in sample_data.columns[:3]:  # Show first 3 columns
                        analysis["sample_values"][col] = sample_data[col].head(3).tolist()
                ...
                if placeholder.get("description"):
                    try:
                        ai_params = self.ai_service.interpret_description_for_tool(
                            ...
                            df_columns=sample_data.columns.tolist() if not sample_data.empty else []
                        )
            ...
            return {
                ...
                } if not sample_data.empty else {"rows": 0, "columns": 0},
        ...
        try:
            ...
            if not template:
                ...
                if not os.path.exists(template.file_path):
                    validation_result["errors"].append(f"Template file not found: {template.file_path}")
                    validation_result["valid"] = False
                else:
                    try:
                        ...
                        placeholders = template_structure.get("placeholders", [])
                        if not placeholders:
                            validation_result["warnings"].append("Template contains no placeholders")
                    except Exception as e:
                        validation_result["errors"].append(f"Error parsing template: {str(e)}")
                        validation_result["valid"] = False
            ...
            if not data_source:
                ...
                if data_source.source_type.value == "sql":
                    if not data_source.connection_string:
                        validation_result["errors"].append("SQL data source requires connection string")
                        validation_result["valid"] = False
                elif data_source.source_type.value == "csv":
                    if not data_source.file_path:
                        validation_result["errors"].append("CSV data source requires file path")
                        validation_result["valid"] = False
                    elif not os.path.exists(data_source.file_path):
                        validation_result["errors"].append(f"CSV file not found: {data_source.file_path}")
                        validation_result["valid"] = False
                elif data_source.source_type.value == "api":
                    if not data_source.api_url:
                        validation_result["errors"].append("API data source requires URL")
                        validation_result["valid"] = False
            ...
            try:
                ...
                if ai_health["status"] != "healthy":
                    validation_result["warnings"].append(f"AI service not healthy: {ai_health.get('message', 'Unknown issue')}")
            except Exception as e:
                validation_result["warnings"].append(f"AI service check failed: {str(e)}")
