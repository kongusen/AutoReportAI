...
def create_ai_provider(token):
    ...
    if response.status_code == 200:
        ...
        print(f"✅ AI Provider创建成功: {provider['provider_name']} (ID: {provider['id']})")
        return provider
    else:
...
def test_ai_provider(token, provider_id):
    ...
    response = requests.post(
        f"{BASE_URL}/ai-providers/{provider_id}/test",
        headers=headers
    )
...
def test_data_source(token, data_source_id):
    ...
    response = requests.post(
        f"{BASE_URL}/data-sources/{data_source_id}/test",
        headers=headers
    )
