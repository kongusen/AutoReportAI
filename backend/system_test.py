    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if providers:
                ...
                # 测试连接
                response = requests.post(f"{BASE_URL}/ai-providers/{provider_id}/test", headers=headers)
                if response.status_code == 200:
                ...
                # 获取模型列表
                response = requests.get(f"{BASE_URL}/ai-providers/{provider_id}/models", headers=headers)
                if response.status_code == 200:
                    models = response.json()
                    print_result(True, f"AI Provider模型列表获取成功，共{len(models)}个模型")
                else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if sources:
                ...
                # 测试连接
                response = requests.post(f"{BASE_URL}/data-sources/{source_id}/test", headers=headers)
                if response.status_code == 200:
                ...
                # 数据预览
                response = requests.get(f"{BASE_URL}/data-sources/{source_id}/preview", headers=headers)
                if response.status_code == 200:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if jobs:
                ...
                # 验证配置
                response = requests.post(f"{BASE_URL}/etl-jobs/{job_id}/validate", headers=headers)
                if response.status_code == 200:
                ...
                # 干运行测试
                response = requests.post(f"{BASE_URL}/etl-jobs/{job_id}/run?dry_run=true", headers=headers)
                if response.status_code == 200:
