...
from app.schemas.enhanced_data_source import EnhancedDataSourceCreate, EnhancedDataSourceUpdate
...
class ComplexScenarioTester:
    ...
    def create_complex_enhanced_source(self, name_suffix: str = "") -> EnhancedDataSource:
        """创建最复杂的增强数据源"""
        source = EnhancedDataSource(
            ...
            wide_table_schema={
                ...
                "columns": [
                    ...
                    {"name": "total_orders", "type": "INTEGER", "nullable": False, "default": 0},
                    {"name": "total_spent", "type": "DECIMAL(10,2)", "nullable": False, "default": 0.0},
    ...
    def create_complex_etl_job(self, enhanced_source_id: int, job_suffix: str = "") -> ETLJob:
        """创建最复杂的ETL作业"""
        job = ETLJob(
    ...
    def test_cascade_operations(self):
        ...
        # 验证关系
        source_with_jobs = self.db.query(EnhancedDataSource).filter_by(id=source.id).first()
        ...
        # 验证所有相关ETL作业已被删除
        remaining_jobs = self.db.query(ETLJob).filter(
            ETLJob.id.in_(job_ids)
        ).all()
    ...
    def test_performance_scenarios(self):
        ...
        # 测试复杂查询性能
        complex_query = self.db.query(EnhancedDataSource).filter(
