...
class BackendTester:
    ...
    def test_data_source_creation(self):
        ...
        print('数据源创建响应:', response.status_code, response.text)
        if response.status_code == 201:
    ...
    def test_task_creation(self):
        ...
        print('任务创建响应:', response.status_code, response.text)
        if response.status_code == 201:
    ...
    def test_dashboard_data(self):
        """测试仪表板数据"""
        response = requests.get(f"{self.base_url}/dashboard/stats", headers=self.auth_headers)
        if response.status_code == 200:
            dashboard_data = response.json()
            # 适配API返回结构
            if isinstance(dashboard_data, dict) and dashboard_data.get('success'):
                data = dashboard_data.get('data', {})
                print(f"   📊 数据源: {data.get('data_sources', 0)}")
                print(f"   📋 模板: {data.get('templates', 0)}")
                print(f"   ⚙️ 任务: {data.get('tasks', 0)}")
            return True
        return False
    ...
    def test_report_generation(self):
        ...
        response = requests.post(f"{self.base_url}/reports/generate", params=params, headers=self.auth_headers)
...
def main():
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            print("请确保后端服务已启动: make run-backend")
            return False
