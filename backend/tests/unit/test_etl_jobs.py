...
def create_test_data_source(
    client: TestClient, db_session: Session, token: str, name_suffix: str = ""
) -> int:
...
def test_update_etl_job_not_found(client: TestClient, db_session: Session) -> None:
    ...
    update_data = {"description": "Updated description", "enabled": True}
...
def test_create_etl_job_invalid_destination_table(
...
def test_create_etl_job_unauthorized(client: TestClient, db_session: Session) -> None:
    ...
    login_data = {"username": "regular_user_etl", "password": "password123"}
    response = client.post("/api/v1/auth/access-token", data=login_data)
...
def test_get_etl_job_status(client: TestClient, db_session: Session):
    ...
    create_response = client.post(
        f"{settings.API_V1_STR}/etl-jobs/", json=etl_job_data, headers=headers
    )
    assert create_response.status_code == 200
    ...
    # Get job status
    response = client.get(
        f"{settings.API_V1_STR}/etl-jobs/{job_id}/status", headers=headers
    )
    assert response.status_code == 200
...
def test_validate_etl_job_configuration(client: TestClient, db_session: Session):
    ...
    response = client.post(
        f"{settings.API_V1_STR}/etl-jobs/{job_id}/validate", headers=headers
    )
    assert response.status_code == 200
...
def test_validate_etl_job_invalid_configuration(
    client: TestClient, db_session: Session
):
...
def test_dry_run_etl_job(client: TestClient, db_session: Session):
    ...
    # Perform dry run
    response = client.post(
        f"{settings.API_V1_STR}/etl-jobs/{job_id}/dry-run", headers=headers
    )
    assert response.status_code == 200
...
def test_list_data_source_tables_not_found(client: TestClient, db_session: Session):
    ...
    response = client.get(
        f"{settings.API_V1_STR}/etl-jobs/data-source/999999/tables", headers=headers
    )
    assert response.status_code == 404
