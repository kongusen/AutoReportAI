...
def test_test_report_pipeline(client: TestClient, db_session: Session):
    ...
    response = client.post(f"{settings.API_V1_STR}/reports/test", headers=headers)
...
def test_validate_report_configuration_missing_template(
    ...
    response = client.post(
        ...
        headers=headers,
        params={"template_id": 999999, "data_source_id": data_source.id},
    )
...
def test_validate_report_configuration_missing_data_source(
    client: TestClient, db_session: Session
):
    ...
    template = crud.template.create(
        ...
        obj_in=schemas.TemplateCreate(
            name="test_report_template", description="Test template"
        ),
        file_path="tests/test_data/sample_files/basic_template.txt",
    ...
    response = client.post(
        f"{settings.API_V1_STR}/reports/validate",
        headers=headers,
        params={"template_id": template.id, "data_source_id": 999999},
    )
...
def test_get_generation_status(client: TestClient, db_session: Session):
    ...
    response = client.get(f"{settings.API_V1_STR}/reports/status/123", headers=headers)
...
def test_generate_report_missing_task(client: TestClient, db_session: Session):
    ...
    response = client.post(
        ...
        headers=headers,
        params={"task_id": 999999, "template_id": 1, "data_source_id": 1},
    )
...
def test_preview_report_data_missing_template(client: TestClient, db_session: Session):
    ...
    response = client.get(
        ...
        headers=headers,
        params={"template_id": 999999, "data_source_id": 1, "limit": 5},
    )
