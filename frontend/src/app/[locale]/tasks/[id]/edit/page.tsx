...
export default function EditTaskPage() {
  ...
  const fetchTask = async () => {
    ...
    try {
      ...
      form.reset({
        ...
        data_source_id: task.data_source_id ? String(task.data_source_id) : '',
      ...
      setSelectedTemplate(task.template ? { ...task.template } : null)
      setSelectedDataSource(task.data_source ? { ...task.data_source } : null)
    } catch {
      // ignore
    } finally {
      setLoading(false)
  ...
  const onSubmit = async (values: TaskFormValues) => {
    ...
    try {
      ...
      await httpClient.put(`/v1/tasks/${taskId}`, payload)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl font-semibold text-gray-900">{t('editTask', 'tasks')}</h1>
                <p className="text-sm text-gray-500">{t('editTaskDescription', 'tasks')}</p>
              </div>
            </div>
